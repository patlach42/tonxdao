...
const createGlobalDisableIblRadianceUniform = () => {
  if (
    !ShaderChunk.envmap_physical_pars_fragment.includes("iblRadianceDisabled")
  ) {
  ...
  ShaderLib.physical.uniforms.iblRadianceDisabled =
    globalIblRadianceDisabledUniform;
...
class TemporalReprojectMaterial extends ShaderMaterial {
  constructor(textureCount = 1) {
    ...
    finalFragmentShader =
      definitions +
      finalFragmentShader.replaceAll("textureCount", textureCount);
    finalFragmentShader = unrollLoops(finalFragmentShader);
    const matches2 = finalFragmentShader.matchAll(
      /accumulatedTexture\[\s*[0-9]+\s*]/g,
    );
...
class TemporalReprojectPass extends Pass {
  constructor(
    ...
    this.renderTarget.texture.forEach(
      (texture, index) =>
        (texture.name = "TemporalReprojectPass.accumulatedTexture" + index),
    );
    ...
    this.fullscreenMaterial.defines.neighborhoodClampRadius = parseInt(
      options.neighborhoodClampRadius,
    );
    this.fullscreenMaterial.defines.depthDistance =
      options.depthDistance.toPrecision(5);
    this.fullscreenMaterial.defines.worldDistance =
      options.worldDistance.toPrecision(5);
    this.fullscreenMaterial.uniforms.fullAccumulate.value =
      options.fullAccumulate;
    this.fullscreenMaterial.uniforms.neighborhoodClampIntensity.value =
      options.neighborhoodClampIntensity;
    ...
    this.fullscreenMaterial.uniforms.cameraMatrixWorld.value =
      camera.matrixWorld;
    this.fullscreenMaterial.uniforms.viewMatrix.value =
      camera.matrixWorldInverse;
    this.fullscreenMaterial.uniforms.cameraPos.value = camera.position;
    this.fullscreenMaterial.uniforms.prevViewMatrix.value =
      camera.matrixWorldInverse.clone();
    this.fullscreenMaterial.uniforms.prevCameraMatrixWorld.value =
      camera.matrixWorld.clone();
    this.fullscreenMaterial.uniforms.prevProjectionMatrix.value =
      camera.projectionMatrix.clone();
    this.fullscreenMaterial.uniforms.prevProjectionMatrixInverse.value =
      camera.projectionMatrixInverse.clone();
    this.fullscreenMaterial.uniforms.velocityTexture.value =
      velocityDepthNormalPass.renderTarget.texture;
    this.fullscreenMaterial.uniforms.depthTexture.value =
      velocityDepthNormalPass.depthTexture;
    this.fullscreenMaterial.defines.inputType =
      (_indexOf = ["diffuseSpecular", "diffuse", "specular"].indexOf(
        options.inputType,
      )) !== null && _indexOf !== void 0
        ? _indexOf
        : 1;
    ...
    this.fullscreenMaterial.defines.confidencePower =
      options.confidencePower.toPrecision(5);
  ...
  setSize(width, height) {
    ...
    (_this$framebufferText = this.framebufferTexture) == null
      ? void 0
      : _this$framebufferText.dispose();
    ...
    for (let i = 0; i < this.textureCount; i++) {
      ...
      const accumulatedTexture =
        (_this$overrideAccumul = this.overrideAccumulatedTextures[i]) !==
          null && _this$overrideAccumul !== void 0
  ...
  render(renderer) {
    ...
    this.fullscreenMaterial.uniforms.projectionMatrix.value.copy(
      this._camera.projectionMatrix,
    );
    this.fullscreenMaterial.uniforms.projectionMatrixInverse.value.copy(
      this._camera.projectionMatrixInverse,
    );
    this.fullscreenMaterial.uniforms.lastVelocityTexture.value =
      this.velocityDepthNormalPass.lastVelocityTexture;
    this.fullscreenMaterial.uniforms.fullAccumulate.value =
      this.options.fullAccumulate &&
      !didCameraMove(
        this._camera,
        this.lastCameraTransform.position,
        this.lastCameraTransform.quaternion,
      );
    ...
    this.fullscreenMaterial.uniforms.prevViewMatrix.value.copy(
    ...
    this.fullscreenMaterial.uniforms.prevCameraPos.value.copy(
...
class TRAAEffect extends Effect {
  ...
  update(renderer, inputBuffer) {
    if (!this.temporalReprojectPass) {
      ...
      this.uniforms.get("accumulatedTexture").value =
...
class CubeToEquirectEnvPass extends Pass {
  ...
  generateEquirectEnvMap(
    ...
    const equirectEnvMap = new DataTexture(
      pixelBuffer,
      width,
      height,
      RGBAFormat,
...
const propsPrimitive = props.filter(
  (key) =>
    typeof gBufferMaterial[key] === "string" ||
    typeof gBufferMaterial[key] === "number",
);
...
class GBufferPass extends Pass {
  ...
  setGBufferMaterialInScene() {
    ...
    const cameraMoved = didCameraMove(
      this._camera,
    ...
    for (const c of this.visibleMeshes) {
      ...
      let [cachedOriginalMaterial, gBufferMaterial] =
        this.cachedMaterials.get(c) || []; // init a new material if the original material changed or if the cached material is missing
...
const workerOnMessage = ({
  data: { width, height, isFloatType, flipY, data },
}) => {
  ...
  updateFrom(map) {
    ...
    return new Promise((resolve) => {
      ...
      this.worker.onmessage = ({
        data: { totalSumValue, marginalDataArray, conditionalDataArray },
      }) => {
        this.dispose();
...
class SSGIPass extends Pass {
  constructor(ssgiEffect, options) {
    ...
    if (ssgiEffect._camera.isPerspectiveCamera)
      this.fullscreenMaterial.defines.PERSPECTIVE_CAMERA = "";
    this.fullscreenMaterial.defines.mode = ["ssgi", "ssr"].indexOf(
      options.mode,
    );
    this.gBufferPass = new GBufferPass(this._scene, this._camera);
    this.fullscreenMaterial.uniforms.gBufferTexture.value =
      this.gBufferPass.texture;
    this.fullscreenMaterial.uniforms.depthTexture.value =
      this.gBufferPass.depthTexture;
  }
  ...
  setSize(width, height) {
    ...
    this.fullscreenMaterial.uniforms.resolution.value.set(
      this.renderTarget.width,
      this.renderTarget.height,
    );
  }
  ...
  render(renderer) {
    ...
    this.fullscreenMaterial.uniforms.cameraNear.value = this._camera.near;
    this.fullscreenMaterial.uniforms.cameraFar.value = this._camera.far;
    this.fullscreenMaterial.uniforms.nearMinusFar.value =
      this._camera.near - this._camera.far;
    this.fullscreenMaterial.uniforms.farMinusNear.value =
      this._camera.far - this._camera.near;
    this.fullscreenMaterial.uniforms.nearMulFar.value =
      this._camera.near * this._camera.far;
    this.fullscreenMaterial.uniforms.accumulatedTexture.value =
      this.ssgiEffect.denoiser.texture;
    this.fullscreenMaterial.uniforms.velocityTexture.value =
      this.ssgiEffect.velocityTexture;
    const bgColor =
      this._scene.background instanceof Color
...
class VelocityDepthNormalPass extends Pass {
  ...
  setVelocityDepthNormalMaterialInScene() {
    ...
    for (const c of this.visibleMeshes) {
      const originalMaterial = c.material;
      let [cachedOriginalMaterial, velocityDepthNormalMaterial] =
      ...
      velocityDepthNormalMaterial.uniforms.normalMap.value =
  ...
  setSize(width, height) {
    ...
    this.renderTarget.setSize(width, height);
    (_this$lastVelocityTex = this.lastVelocityTexture) == null
      ? void 0
      : _this$lastVelocityTex.dispose();
...
/* eslint-disable camelcase */
const finalFragmentShader$1 = fragmentShader$5.replace(
  "#include <gbuffer_packing>",
  gbuffer_packing,
);
...
class PoissonDenoisePass extends Pass {
  constructor(camera, textures, options = defaultPoissonBlurOptions) {
    ...
    const fragmentShader = unrollLoops(
      finalFragmentShader$1.replaceAll("textureCount", textureCount),
    );
    this.fullscreenMaterial = new ShaderMaterial({
      fragmentShader,
      vertexShader,
    ...
    this.renderTargetA = new WebGLMultipleRenderTargets(
      1,
      1,
      textureCount,
      renderTargetOptions,
    );
    this.renderTargetB = new WebGLMultipleRenderTargets(
      1,
      1,
      textureCount,
      renderTargetOptions,
    ...
    this.renderTargetB.texture[0].name =
      "PoissonDenoisePass." + (isTextureSpecular[0] ? "specular" : "diffuse");

    if (textureCount > 1) {
      this.renderTargetB.texture[1].name =
        "PoissonDenoisePass." + (isTextureSpecular[1] ? "specular" : "diffuse");
    }
  ...
  setGBufferPass(gBufferPass) {
    ...
    this.fullscreenMaterial.uniforms.depthTexture.value =
      gBufferPass.renderTarget.depthTexture;
  }
...
class Denoiser {
  constructor(scene, camera, texture, options = defaultDenosierOptions) {
    ...
    this.options = options;
    this.velocityDepthNormalPass =
      (_options$velocityDept = options.velocityDepthNormalPass) !== null &&
      _options$velocityDept !== void 0
    ...
    if (
      this.options.denoiseMode === "full" ||
      this.options.denoiseMode === "denoised"
    ) {
      var _options$gBufferPass;

      this.denoisePass = new PoissonDenoisePass(camera, textures, options);
      this.denoisePass.setGBufferPass(
        (_options$gBufferPass = options.gBufferPass) !== null &&
          _options$gBufferPass !== void 0
          ? _options$gBufferPass
          : this.velocityDepthNormalPass,
      );
      this.temporalReprojectPass.overrideAccumulatedTextures =
        this.denoisePass.renderTargetB.texture;
    }

    const composerInputTextures =
      (_this$denoisePass$tex =
        (_this$denoisePass = this.denoisePass) == null
          ? void 0
          : _this$denoisePass.texture) !== null &&
      _this$denoisePass$tex !== void 0
        ? _this$denoisePass$tex
        : textures;
  ...
  setSize(width, height) {
    ...
    (_this$denoisePass2 = this.denoisePass) == null
      ? void 0
      : _this$denoisePass2.setSize(width, height);
    (_this$denoiserCompose = this.denoiserComposePass) == null
      ? void 0
      : _this$denoiserCompose.setSize(width, height);
  }
  ...
  dispose() {
    ...
    this.temporalReprojectPass.dispose();
    (_this$denoisePass3 = this.denoisePass) == null
      ? void 0
      : _this$denoisePass3.dispose();
    (_this$denoiserCompose2 = this.denoiserComposePass) == null
      ? void 0
      : _this$denoiserCompose2.dispose();
  }
  ...
  render(renderer, inputBuffer = null) {
    ...
    if (this.isOwnVelocityDepthNormalPass)
      this.velocityDepthNormalPass.render(renderer);
    this.temporalReprojectPass.render(renderer);
    ...
    (_this$denoisePass4 = this.denoisePass) == null
      ? void 0
      : _this$denoisePass4.render(renderer);
    (_this$denoiserCompose4 = this.denoiserComposePass) == null
      ? void 0
      : _this$denoiserCompose4.render(renderer);
  }
}
...
class SSGIEffect extends Effect {
  constructor(composer, scene, camera, options) {
    ...
    if ((_scene$fog = scene.fog) != null && _scene$fog.isFogExp2)
      defines.set("FOG_EXP2", "");
  ...
  makeOptionsReactive(options) {
    ...
    const ssgiPassFullscreenMaterialUniforms =
      this.ssgiPass.fullscreenMaterial.uniforms;
          ...
          switch (key) {
            ...
            case "specularPhi":
              if (
                ...
                this.denoiser.denoisePass.fullscreenMaterial.uniforms[
                  key
                ].value = value;
                this.reset();
              }
            ...
            case "outputTexture":
              ...
              if (typeof value === "string") {
                if (this.gBufferDebugPass === undefined) {
                  this.gBufferDebugPass = new GBufferDebugPass(
                    this.ssgiPass.gBufferPass.texture,
                  );
                  this.gBufferDebugPass.setSize(
                    this.lastSize.width,
                ...
                this.gBufferDebugPass.fullscreenMaterial.uniforms.mode.value =
                  mode;
                this.outputTexture = this.gBufferDebugPass.texture;
              } else if (
                this.gBufferDebugPass !== undefined &&
                this.outputTexture !== this.gBufferDebugPass.texture
              ) {
              ...
              this.uniforms.get("isDebug").value =
  ...
  setSize(width, height, force = false) {
    ...
    (_this$gBufferDebugPas = this.gBufferDebugPass) == null
      ? void 0
      : _this$gBufferDebugPas.setSize(width, height);
  ...
  dispose() {
    ...
    (_this$cubeToEquirectE2 = this.cubeToEquirectEnvPass) == null
      ? void 0
      : _this$cubeToEquirectE2.dispose();
  ...
  update(renderer, inputBuffer) {
    ...
    const sceneBuffer = this.isUsingRenderPass
      ? inputBuffer
    ...
    this.ssgiPass.fullscreenMaterial.uniforms.directLightTexture.value =
      sceneBuffer.texture;
    this.ssgiPass.render(renderer);
    (_this$gBufferDebugPas2 = this.gBufferDebugPass) == null
      ? void 0
      : _this$gBufferDebugPas2.render(renderer);
    this.denoiser.render(renderer, inputBuffer);
    this.uniforms.get("inputTexture").value =
      (_this$outputTexture$ = this.outputTexture[0]) !== null &&
      _this$outputTexture$ !== void 0
        ? _this$outputTexture$
        : this.outputTexture;
    this.uniforms.get("sceneTexture").value = sceneBuffer.texture;
    this.uniforms.get("depthTexture").value =
      this.ssgiPass.gBufferPass.depthTexture; // update the fog uniforms
...
class AOPass extends Pass {
  ...
  render(renderer) {
    ...
    this.fullscreenMaterial.uniforms.frame.value =
      (this.fullscreenMaterial.uniforms.frame.value + spp) % 4096;
    this.fullscreenMaterial.uniforms.cameraNear.value = this._camera.near;
    this.fullscreenMaterial.uniforms.cameraFar.value = this._camera.far;
...
const finalFragmentShader = fragmentShader$4.replace(
  "#include <hbao_utils>",
  hbao_utils,
);
...
class AOEffect extends Effect {
  constructor(composer, camera, scene, aoPass, options = defaultAOOptions) {
    ...
    this.aoPass.fullscreenMaterial.uniforms.depthTexture.value =
      composer.depthTexture;
    ...
    if (options.useNormalPass || options.normalTexture) {
      ...
      const normalTexture =
        (_options$normalTextur = options.normalTexture) !== null &&
        _options$normalTextur !== void 0
          ? _options$normalTextur
          : this.normalPass.texture;
      this.aoPass.fullscreenMaterial.uniforms.normalTexture.value =
        normalTexture;
      this.aoPass.fullscreenMaterial.defines.useNormalTexture = "";
    }

    this.PoissonDenoisePass = new PoissonDenoisePass(
      camera,
      this.aoPass.texture,
      composer.depthTexture,
      {
      ...
      Object.defineProperty(this, key, {
        ...
        set(value) {
          ...
          switch (key) {
            ...
            case "depthPhi":
            case "normalPhi":
              this.PoissonDenoisePass.fullscreenMaterial.uniforms[key].value =
  ...
  setSize(width, height) {
    ...
    (_this$normalPass = this.normalPass) == null
      ? void 0
      : _this$normalPass.setSize(width, height);
    this.aoPass.setSize(
  ...
  update(renderer) {
    ...
    const hasTRAA = this.composer.passes.some((pass) => {
      ...
      return (
        pass.enabled &&
        !pass.skipRendering &&
        ((_pass$effects = pass.effects) == null
          ? void 0
    ...
    if (
      hasTRAA &&
      !("animatedNoise" in this.aoPass.fullscreenMaterial.defines)
    ) {
    ...
    (_this$normalPass2 = this.normalPass) == null
      ? void 0
      : _this$normalPass2.render(renderer);
...
class TAAPass extends Pass {
  ...
  setSize(width, height) {
    ...
    this.framebufferTexture.needsUpdate = true;
    this.fullscreenMaterial.uniforms.acculumatedTexture.value =
      this.framebufferTexture;
  ...
  render(renderer, inputBuffer) {
    ...
    this.needsUpdate = false;
    const cameraNotMovedFrames =
      this.fullscreenMaterial.uniforms.cameraNotMovedFrames.value;
    ...
    this.fullscreenMaterial.uniforms.cameraNotMovedFrames.value = cameraMoved
      ? 0
      : (cameraNotMovedFrames + 1) % 4096;
    this.lastCameraPosition.copy(this._camera.position);
    this.lastCameraQuaternion.copy(this._camera.quaternion);
...
class SparkleEffect extends Effect {
  ...
  update() {
    ...
    this.uniforms
      ...
      .value.copy(this._camera.projectionMatrix);
    this.uniforms
      .get("projectionMatrixInverse")
      .value.copy(this._camera.projectionMatrixInverse);
    view &&
